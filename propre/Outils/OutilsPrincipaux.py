...
#recuperation du text d'un fichier en arbreXML
def recuperationArbreText(url):
    #recuperation de l'arbe xml
    tree = ET.parse(url)
    root=tree.getroot()
...
def formatage(text):
    ...
    #mise en forme des noms des meurtrier pour les listes
    #voir si on peux recuperer le meurtirer comme ca
    ...
    for sent in liststrRoot:
        ...
        for i,w in enumerate(wordToken):
            #on ne prend pas la ponctuation
            if w.isalpha():
                if w=="ref":
                    debut=i-1
                    continueF=False
...
#utilisation des phrase de test
def sentTest(para):
    strRootList=[]
    if(para == 1):
...
#creation d'un arbre a partir du texte taggé
#utilisation d'un pattern pour trouver ce qu'on veux
